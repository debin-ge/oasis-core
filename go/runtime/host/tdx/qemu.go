...
func (q *qemuProvisioner) getSandboxConfig(rtCfg host.Config, conn sandbox.Connector, _ string) (process.Config, error) {
	...
	cid := rtCfg.Extra.(*QemuExtraConfig).CID

	// Configure the CID on the VSOCK connector.
	vs, ok := conn.(*vsockConnector)
	if !ok {
		return process.Config{}, fmt.Errorf("VSOCK connector is required")
	}
	vs.cid = cid
	...
	if tdxCfg.HasKernel() {
		...
		if tdxCfg.HasStage2() {
			...
			cfg.Args = append(cfg.Args,
				// Stage 2 disk.
				"-drive", fmt.Sprintf("format=raw,file=%s,if=none,id=virtio-disk1,read-only=on", stage2Image),
	...
	case false:
		cfg.Args = append(cfg.Args,
			"-netdev", "user,id=nic0_td,restrict=y",
		)
	}
	cfg.Args = append(cfg.Args,
...
func (q *qemuProvisioner) updateCapabilityTEE(ctx context.Context, hp *sandbox.HostInitializerParams) (cap *node.CapabilityTEE, aerr error) {
	defer func() {
		sgxCommon.UpdateAttestationMetrics(hp.Runtime.ID(), aerr)
	}()
	...
	quoteBundle, err := q.qs.ResolveQuote(ctx, rawQuote, quotePolicy.PCS)
	if err != nil {
...
func NewQemu(cfg QemuConfig) (host.Provisioner, error) {
	...
	q.qs = pcs.NewCachingQuoteService(cfg.PCS, cfg.CommonStore, q.logger)
...
func (vs *vsockConnector) ConfigureSandbox(_ *process.Config) error {
	return nil
}

// Connect establishes a connection to the runtime.
func (vs *vsockConnector) Connect(_ process.Process) (net.Conn, error) {
	var (
		conn *vsock.Conn
		err  error
	)
	...
	if err = backoff.Retry(op, backoff.WithContext(sched, ctx)); err != nil {
